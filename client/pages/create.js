...
const SubmissionForm = () => {
			...
			if(files){
				...
				.then((res)=>{
					...
					.catch((err) => {
						setResultText({
							type: "error",
							message: "Some error occurred while posting article. Please contact administrator."
						})
						console.log(err);
					})
					.finally(()=>{
	...
	return (
		<div className={styles.submissioncontainer}>
			...
			<button className={`${loading&&styles.loading} ${styles.submitbtn}`} onClick={submitHandler}>
				{loading?(
					`Loading...`
				):`Submit`}
			</button>
			<p className={resultText&&(resultText.type==="success"?styles.successText:styles.errorText)}>
					{resultText.message&&(
						resultText.message
					)}
			</p>
